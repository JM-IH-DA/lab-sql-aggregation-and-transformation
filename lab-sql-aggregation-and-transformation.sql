...
SELECT
    ...
    ROUND(AVG(length)%60), 'min') AS average_duration
FROM film;
...
-- "DAYOFWEEK function, considering from 1: Sunday to 7: Saturday".
SELECT
	*,
	CASE
...
SELECT 
	title,
    IFNULL (rental_duration, 'not_available') AS rental_duration
FROM film
ORDER BY title ASC;
...
SELECT DISTINCT COUNT(title) AS total_released
FROM film;
